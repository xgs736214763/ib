<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:62:"G:\wamp64\www\ib5/application/index\view\ibeacon\position.html";i:1504503093;s:59:"G:\wamp64\www\ib5/application/index\view\Public\header.html";i:1501215050;s:59:"G:\wamp64\www\ib5/application/index\view\Public\footer.html";i:1501215044;}*/ ?>
<!DOCTYPE html>
<html lang="en">
				...
				<form action="" method="get">
					<table class="content">
						<tr style="display: none;">
							<td>
								<input type="text" name="location" class="input-normal control-text" id="location" value="<?php echo isset($_GET['mac'])?$_GET['mac']:'';?>" />
						</tr>
						<tr>
							<td>Ibeacon名称：</td>
							<td> <input type="text" name="ibname" class="input-normal control-text" id="ibname" class="store" value="<?php echo $apinfo['ibname']; ?>" /></td>
						</tr>
						<tr>
							<td>mac：</td>
						...
						<tr>
							<td>uuid：</td>
							<td> <input type="text" name="uuid" class="input-normal control-text" id="uuid" class="store" value="<?php echo $apinfo['uuid']; ?>" /></td>
						</tr>
						<tr>
							<td>major：</td>
							<td> <input type="text" name="major" class="input-normal control-text" id="major" class="store" value="<?php echo $apinfo['major']; ?>" /></td>
						</tr>
						<tr>
							<td>minor：</td>
							<td> <input type="text" name="minor" class="input-normal control-text" id="minor" class="store" value="<?php echo $apinfo['minor']; ?>" /></td>
						</tr>
						<tr>
							<td>厂商：</td>
	...
	<script type="text/javascript">
		...
		for(i=0;i<muCnt;i++){
				...
				{
					//alert(aplist[i].Xcor);
					mymap.on('locationfound', onLocationFound(<?php echo $map['floor_img_Y']; ?>-aplist[i].Ycor/$coefficienty, aplist[i].Xcor/$coefficientx,aplist[i].mac,aplist[i].ibname,aplist[i].id));
				//  mymap.on('locationfound', onLocationFound(mulist[i].locationY, mulist[i].locationX,mulist[i].mac,mulist[i].devType));
				  }else
				{
					mymap.on('locationfound', ofLocationFound(<?php echo $map['floor_img_Y']; ?>-aplist[i].Ycor/$coefficienty, aplist[i].Xcor/$coefficientx,aplist[i].mac,aplist[i].ibname,aplist[i].id));
...
function savedata(lat,lng,mac,id){
	...
	$.ajax({
		...
		dateType:'json',
		data:{'lat':lat,'lng':lng,'mac':mac,'maxposX':<?php echo $map['floor_img_X']; ?>,'id':id},
                 ...
                 buttons:[
                  {
                    ...
                    handler : function(){
                    		$floorId = document.getElementById('floor_id').value;
												...
												$minor = $('#minor').val();
                        $.ajax({
                                url:'<?php echo Url('index/ibeacon/saveapinfo'); ?>',
                            type:'post',
